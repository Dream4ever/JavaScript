...
JavaScript 中可以用 `this` 关键字来引用全局对象，但没有方法引用存放局部变量的对象。这种存放局部变量的对象所特有的性质，是对我们不可见的内部实现。但是！可以通过闭包在局部变量的作用域之外使用它，具体请看 [MDN 上关于闭包的文档](https://hewei.in/closure/mdn-closure.html)。
