...
翻译了 MDN 上介绍闭包（Closure）这个概念的一篇文章，以供学习。几年之后再来看，不断进步的 JavaScript 语法和各种现代化开发框架，将很多这类问题都封装起来了，但是对于新人来说，如果遇到这类问题，可能更难找到问题的源头。作为一名开发者，对工具的熟练掌握只是最基础的，知其然更要知其所以然，才能够在技术不断变化的这个时代，夯实自己的基础，以不变应万变。
...
在后两段示例代码中，所有对象都共享继承的原型，所以就不需要在每次创建对象的时候都出现一次方法的定义，这样就不用消耗额外的硬件资源了。可以查看[《Details of the Object Model》](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Details_of_the_Object_Model)以了解更深入的内容。
